...
function serverAssets(nitro) {
  if (nitro.options.dev) {
    return dynamicVirtual({ "#nitro/virtual/server-assets": getAssetsDev(nitro) });
  }
...
const getRollupConfig = (nitro) => {
  ...
  rollupConfig.plugins.push(handlers(() => {
    ...
    if (nitro.options.serveStatic) {
      handlers2.unshift({ route: "", handler: "#nitro/static" });
    }
...
export const plugins = [
  ...
  rollupConfig.plugins.push(alias({
    entries: resolveAliases({
      "#nitro": runtimeDir,
      "#build": nitro.options.dev && isWindows ? pathToFileURL(nitro.options.buildDir).href : nitro.options.buildDir,
...
function scanRoutes(nitro, dir, prefix = "/") {
  return scanServerDir(nitro, dir, (file) => {
    let route = file.path.replace(/\.[a-zA-Z]+$/, "").replace(/index$/, "").replace(/\[...\]/g, "**").replace(/\[([a-zA-Z]+)\]/g, ":$1");
...
async function writeRoutes$1(nitro) {
  ...
  await writeFile(resolve(nitro.options.output.serverDir, "package.json"), JSON.stringify({
    private: true,
    type: "module",
    main: "./index.mjs",
    dependencies: {
