      ...
      <div class="space-y-10">
        <div v-for="review in reviews" :key="review.id" class="flex flex-col sm:flex-row">
          <div class="mt-6 order-2 sm:mt-0 sm:ml-16">
            <h3 class="text-sm font-medium text-white">{{ review.title }}</h3>
            <p class="sr-only">{{ review.rating }} out of 5 stars</p>

            <div class="mt-3 space-y-6 text-sm text-white" v-html="review.content" />
          </div>
          ...
          <div class="order-1 flex items-center sm:flex-col sm:items-start">
            ...
            <div class="ml-4 sm:ml-0 sm:mt-4">
              ...
              <div class="mt-2 flex items-center">
                <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating" :class="[review.rating > rating ? 'text-white' : 'text-gray-600', 'h-5 w-5 flex-shrink-0']" aria-hidden="true" />
              </div>
            </div>
          </div>
        </div>
